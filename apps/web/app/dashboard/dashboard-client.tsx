...
export type DashboardClientProps = {
  /** From server `searchParams` — avoids a blank first paint before `useEffect` reads the URL. */
  autoJudge: boolean;
};

export function DashboardClient({ autoJudge }: DashboardClientProps) {
  const {
    vault,
    proposal,
    decision,
  ...
  const [showJudgeCurtain, setShowJudgeCurtain] = useState(autoJudge);
  ...
  useEffect(() => {
    ...
    void runJudgeDemo90s().finally(() => {
      setShowJudgeCurtain(false);
      if (typeof window !== "undefined") {
        window.history.replaceState({}, "", "/dashboard");
      }
    });
  }, [autoJudge, runJudgeDemo90s]);
  ...
  return (
    <>
      {showJudgeCurtain ? (
        <div
          className="fixed inset-0 z-[100] flex flex-col items-center justify-center gap-4 bg-[#030712]/92 px-6 text-center backdrop-blur-sm"
          role="status"
          aria-live="polite"
        >
          <p className="font-mono text-[10px] uppercase tracking-[0.35em] text-cyan-400/90">Judge demo</p>
          <p className="max-w-md text-lg font-semibold text-slate-100">Running full 90s sequence…</p>
          <p className="max-w-sm text-sm leading-relaxed text-slate-400">
            Vault bind → risky intent → constitution <span className="text-rose-300">REJECT</span> → stress → compliant
            path → <span className="text-cyan-300">ALLOW</span> → execution log.
          </p>
          {error ? <p className="max-w-md font-mono text-xs text-rose-400">{error}</p> : null}
        </div>
      ) : null}
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm text-slate-400">
                ...
                → stress → compliant intent → <span className="text-cyan-300">ALLOW</span> → execution log. Same sequence
                as{" "}
